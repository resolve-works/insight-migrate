...
DROP FUNCTION IF EXISTS private.set_owner();
DROP FUNCTION IF EXISTS private.set_updated_at();
DROP FUNCTION IF EXISTS private.inode_path(bigint);
DROP FUNCTION IF EXISTS private.set_inode_path();

CREATE OR REPLACE FUNCTION set_owner() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
DECLARE
