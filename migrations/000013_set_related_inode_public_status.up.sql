...
-- When updating a inode, mark all its children to the same public state as the inode itself
CREATE OR REPLACE FUNCTION private.set_child_inodes_public_status()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER set_parent_inodes_public_status
    AFTER INSERT OR UPDATE OF is_public ON private.inodes
    FOR EACH ROW
    EXECUTE FUNCTION private.set_parent_inodes_public_status();
